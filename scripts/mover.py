...
def plan_pick_and_place(req):
    ...
    rospy.loginfo("received service !!!!!!!!")

    group_name = "xarm6"
    move_group = moveit_commander.MoveGroupCommander(group_name)

    arm.allow_replanning(True)
    rospy.init_node('move group start !!!----')
    current_robot_joint_configuration = req.joints_input.joints


    rospy.init_node('planning START !!!!-----------')
    ...
    rospy.init_node('planning succeed !!!!-----------')
...
def moveit_server():
    ...
    rospy.loginfo("-----------------------------start moveit_server !!!!!-------------------")
    s = rospy.Service('xarm_moveit', MoverService, plan_pick_and_place)
